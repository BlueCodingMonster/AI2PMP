...
export default function OperationLedgerManager({ records, versions, people }: { records: RecordItem[]; versions: { products: VersionOption[]; projects: VersionOption[] }; people: Person[] }) {
  ...
  return <div className="space-y-5">
    ...
    <style jsx global>{`.control{width:100%;min-height:2.5rem;border:1px solid var(--border);border-radius:.5rem;background:var(--input);padding:.5rem .75rem;color:white;font-size:.875rem;outline:none}.control:focus{border-color:#6366f1;box-shadow:0 0 0 1px rgba(99,102,241,.2)}.control:disabled{cursor:not-allowed;opacity:.5}`}</style>
  </div>;
}
