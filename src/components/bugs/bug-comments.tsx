...
import { addBugComment } from "@/actions/bugs";
...
export default function BugComments({ bugId, comments }: CommentsProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    startTransition(async () => {
      try {
        const res = await addBugComment(bugId, content.trim());
        if (res.success) {
          setContent("");
          router.refresh();
        } else {
          setError(res.error || "回复失败，请重试");
        }
      } catch (err) {
  ...
  return (
    <div className="space-y-4">
      ...
      <form onSubmit={handleSubmit} className="flex gap-2">
        <input
          type="text"
          placeholder="说明缺陷的原因排查进展，或沟通测试细节..."
          value={content}
